...
/**
 * Realization of the {@code AdvancedIP} interface.
 *
 * @author Daniil Moshnikov
 * @see AdvancedIP
 */
...
public class IterativeParallelism implements AdvancedIP {
    ...
    /**
     * Defaul contructor with mapper.
     * @param mapper {@code ParallelMapper}
     */
     ...
     */
    private <T> T max(final Stream<T> stream, final Comparator<? super T> comparator) {
     ...
     * @return values reduced by provided monoid or {@link AdvancedIP.Monoid#getIdentity() identity} if not values specified
     * @throws InterruptedException if executing thread was interrupted
     */
    @Override
    public <T> T reduce(final int threads, final List<T> values, final AdvancedIP.Monoid<T> monoid) throws InterruptedException {
        return parallelReduce(threads, values,
     ...
     */
    @Override
    public <T, R> R mapReduce(final int threads, final List<T> values, final Function<T, R> lift, final AdvancedIP.Monoid<R> monoid) throws InterruptedException {
        return parallelReduce(threads, values,
     ...
     *
     * @param threadCount amount of concurrent threads
     * @param values       values to reduce
     ...
     * @see Thread
     */
    private <T, M, R> R parallelReduce(final int threadCount, final List<T> values,
                                       final Function<Stream<T>, M> threadReduce,
                                       final Function<Stream<M>, R> resReduce) throws InterruptedException {
        final List<Stream<T>> pieces = split(values, threadCount);
        final List<M> res = mapper == null
                ? defaultMap(threadReduce, pieces)
                : mapper.map(threadReduce, pieces);
        return resReduce.apply(res.stream());
    }

    private <T, M> List<M> defaultMap(Function<Stream<T>, M> threadReduce, List<Stream<T>> pieces) throws InterruptedException {
        final List<Thread> threads = new ArrayList<>();
        List<M> res = new ArrayList<>(Collections.nCopies(pieces.size(), null));
        for (int i = 0; i < pieces.size(); i++) {
            final int finalI = i;
            final Thread thread = new Thread(() -> res.set(finalI, threadReduce.apply(pieces.get(finalI))));
            threads.add(thread);
            thread.start();
        }
        joinThreads(threads);
    ...
    private <T> List<Stream<T>> split(final List<T> values, final int blocks) {
        ...
        final List<Stream<T>> pieces = new ArrayList<>();
        int start = 0;
        for (int i = 0; i < blocks; i++) {
            ...
            if (start == end) { // no empty pieces are processed
                break;
            }
            pieces.add(values.subList(start, end).stream());
            start = end;
        }
        return pieces;
    }
     ...
     */
    private <T> List<T> collectToList(final Stream<? extends List<? extends T>> stream) {
