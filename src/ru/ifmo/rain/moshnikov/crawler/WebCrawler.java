package ru.ifmo.rain.moshnikov.crawler;
...
public class WebCrawler implements Crawler {
    ...
    public Result download(String url, int depth) {
        return new WebDownloader(url).run(depth);
    }
        ...
        private Result run(int depth) {
            ...
            for (int i = 0; i < depth; i++) {
                ...
                level.stream()
                        ...
                        .forEach(s -> enqueueDownload(s, toExtract, phaser));
                phaser.arriveAndAwaitAdvance();
            }
            return new Result(List.copyOf(downloaded), failed);
        }

        private void enqueueDownload(String url, int toExtract, Phaser phaser) {
            String host;
            try {
